...
/* finish every generated episode at a complete sentence */
(()=>{
  const source=trimTo;
  trimTo=(text,max)=>{
    const cut=source(text,max);
    let end=-1;
    for(let i=cut.length-1;i>=0;i--){if(/[.!?]/.test(cut[i])&&nonSpace(cut.slice(0,i+1))>=20000){end=i;break}}
